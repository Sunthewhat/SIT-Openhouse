...
import { sendEventFormAPI, sendQueueEventFormAPI } from '@/api/workshop/sendForm';
...
const ConfirmationPopup: FC<PopupProps> = ({ isVisible, onClose, workshops, isQueueEvent }) => {
	...
	const handleSendForm = async () => {
		...
		const res = isQueueEvent
			? await sendQueueEventFormAPI(payload)
			: await sendEventFormAPI(payload);
		if (res.status) {
		...
		<Modal isOpen={isVisible} setIsOpen={onClose}>
			...
			>
				...
				<h2 className='text-xl lg:text-[30px] font-bold text-[#1C3FB7] text-center mb-4'>
					ยืนยันการ{isQueueEvent ? `จอง` : `ลงทะเบียน`}กิจกรรม
				</h2>
				<p className='font-inter text-base lg:text-[20px] font-semibold leading-[30px] text-left text-[#1C3FB7] border-b border-[#DFE4EA] pb-3 mt-[15px]'>
					กิจกรรมที่{isQueueEvent ? `จอง` : `ลงทะเบียน`}
				</p>
				...
				<p className='font-inter text-base lg:text-[20px] font-semibold leading-[30px] text-left text-[#1C3FB7] border-b border-[#DFE4EA] pb-3 mt-[15px]'>
					ข้อมูลผู้{isQueueEvent ? `จอง` : `ลงทะเบียน`}
				</p>
				...
				<div className='flex justify-between gap-10 items-center mt-4'>
					...
					<button
						className={`text-white text-sm lg:text-base font-semibold lg:py-2 lg:px-4 rounded-md w-full md:w-[177px] h-[50px] ${
							isQueueEvent ? 'bg-yellow-500' : 'bg-[#1C3FB7]'
						}`}
						onClick={handleSubmit}
					>
						ยืนยันการ{isQueueEvent ? `จอง` : `ลงทะเบียน`}
					</button>
				</div>
			</div>
			{!isQueueEvent ? (
			...
			{!isQueueEvent ? (
				...
				<SuccessQueueModal isOpen={isSuccessModalOpen} onClose={setIsSuccessModalOpen} />
			)}
		</Modal>
	);
};
	...
	return (
		<Modal isOpen={isOpen} setIsOpen={onClose}>
			<div
				...
				onClick={(e) => e.stopPropagation()}
			>
				<p className='text-yellow-500 font-bold text-xl text-center'>ยืนยันการจองหรือไม่</p>
				<div className='text-[#637381] text-sm text-center my-8 px-7'>
					<span className='font-semibold'>
						หากใส่อีเมลไม่ถูกต้อง ระบบจะไม่สามารถส่งอีเมลข้อมูลการจองให้ท่านได้
					</span>
					<br />
					<span>
						<p>ระบบจะส่งอีเมลข้อมูลการจองให้ท่านผ่านทางอีเมล</p>
					</span>
				</div>
				<div className='grid grid-cols-4 gap-3'>
					<button
	...
	return (
		<Modal isOpen={isOpen} setIsOpen={onClose}>
			<div
				...
				onClick={(e) => e.stopPropagation()}
			>
				<p className='text-yellow-500 font-bold text-xl text-center'>การจองสำเร็จ</p>
				<div className='flex justify-center'>
					<Image src={MailImage} alt='MailImage' className='w-24' />
				</div>
				<p className='text-[#637381] text-sm mt-6 text-center'>
					ระบบได้ส่งอีเมลข้อมูลการจองของท่านผ่านทางอีเมลแล้ว
				</p>
				<div className='grid grid-cols-3 gap-3 mt-6'>
					<button
						className='bg-yellow-500 col-start-2 text-white text-sm lg:text-base font-semibold py-1 lg:py-2 lg:px-4 rounded-md w-fullh-[50px]'
						onClick={() => {
							onClose(false);
							window.location.reload();
						}}
